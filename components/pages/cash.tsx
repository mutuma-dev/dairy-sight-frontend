 ...
 * - Uses SILENT polling every 2 seconds (no blinking / no reload UI)
 * - Shows loading indicator ONLY on first load
 */
...
export default function Cash() {
  ...
  /**
   * Fetch cash payments
   * @param silent - when true, does NOT trigger loading UI (background refresh)
   */
  const fetchCashPayments = async (silent = false) => {
    try {
  ...
  useEffect(() => {
    ...
    fetchCashPayments(false)

    // Silent refresh every 2 seconds (no blinking)
    const interval = setInterval(() => {
      fetchCashPayments(true)
    }, 2000)
